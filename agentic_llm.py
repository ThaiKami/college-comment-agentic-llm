...
def final_inference(client: OpenAI, config: PipelineConfig, state: PipelineState, comment: str) -> Dict[str, Any]:
    system = "You are a careful analyst for student comments. Output JSON only."
...
def evaluate_report(client: OpenAI, config: PipelineConfig, state: PipelineState, comment: str, report: Dict[str, Any]) -> Dict[str, Any]:
...
def detect_error(client: OpenAI, config: PipelineConfig, state: PipelineState, comment: str, report: Dict[str, Any], evaluation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    user = (
        ...
        f"{json.dumps(report, ensure_ascii=True)}\n\n"
        "Evaluation JSON:\n"
        f"{json.dumps(evaluation, ensure_ascii=True)}"
    )
    return parse_json(call_llm(client, config, system, user))


def infer_reason(client: OpenAI, config: PipelineConfig, state: PipelineState, error: Dict[str, Any]) -> Dict[str, Any]:
    ...
    user = (
        ...
        "Error JSON:\n"
        f"{json.dumps(error, ensure_ascii=True)}"
    )
    return parse_json(call_llm(client, config, system, user))


def evolve_instructions(client: OpenAI, config: PipelineConfig, state: PipelineState, reason: Dict[str, Any]) -> Dict[str, Any]:
    ...
    user = (
        ...
        "Root Cause JSON:\n"
        f"{json.dumps(reason, ensure_ascii=True)}"
    )
    return parse_json(call_llm(client, config, system, user))


def conditional_rules(client: OpenAI, config: PipelineConfig, state: PipelineState, instruction: Dict[str, Any]) -> Dict[str, Any]:
...
def augment_report(client: OpenAI, config: PipelineConfig, state: PipelineState, comment: str, rules: List[str]) -> Dict[str, Any]:
    system = "You revise reports following rules. Output JSON only."
    updated_state = PipelineState(rag=state.rag, rules=state.rules + rules)
    chunks = state.rag.retrieve(comment, config.rag_config.top_k)
...
def select_best(client: OpenAI, config: PipelineConfig, state: PipelineState, comment: str, original: Dict[str, Any], revised: Dict[str, Any]) -> Dict[str, Any]:
    ...
    user = (
        ...
        f"{json.dumps(original, ensure_ascii=True)}\n\n"
        "Revised Report JSON:\n"
        f"{json.dumps(revised, ensure_ascii=True)}"
    )
    return parse_json(call_llm(client, config, system, user))
...
def run_pipeline(client: OpenAI, config: PipelineConfig, state: PipelineState, comment: str) -> Dict[str, Any]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Agentic LLM pipeline for student comments")
    ...
    parser.add_argument("--embedding-model", default=os.getenv("VLLM_EMBEDDING_MODEL", ""), help="Embedding model")
    ...
    parser.add_argument("--update-kb", default=None, help="Optional path to append new rules")
...
def main() -> None:
    ...
    if not args.embedding_model:
        raise SystemExit("Missing embedding model. Set --embedding-model or VLLM_EMBEDDING_MODEL.")
    ...
    rag = RagIndex(client, config.embedding_model, config.rag_config)
    rag.build(knowledge_text)
    state = PipelineState(rag=rag, rules=[])
    ...
    with open(args.output, "w", encoding="utf-8") as handle:
        json.dump(outputs, handle, ensure_ascii=True, indent=2)
